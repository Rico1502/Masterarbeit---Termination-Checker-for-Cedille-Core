...
-- Converts a TermP to an ASCII String
toString :: TermP -> String
toString = go 0 where
  -- Type
  go d (Term Typ)
    = "*"
...
annotate :: TermP -> Ann
annotate = go [] where
  go :: [(String, (TermP, TermP))] -> TermP -> Ann
  ...
  ex :: String -> TermP -> [(String, (TermP, TermP))] -> (TermP -> TermP) -> TermP -> Ann
  ex nam typ ctx bod = \var -> go ((nam, (var, typ)) : ctx) (bod (Term (Var nam)))

-- Checks if two terms are syntactically equal
equals :: TermP -> TermP -> Bool
equals = go 0 where
  go :: Int -> TermP -> TermP -> Bool
